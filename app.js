...
var cors = require("cors"); //Cors is used to relax security and used to pre send calls 
...
app.get("/patients/:id", (req, res) => { //calling patients from id
  con.query(`select * from patients where id = ${req.params.id}`,
    (err, result) => {
      if (err) res.statusCode(404).send(err.message);
      res.send({
        status: "success",
...
app.delete("/patients/:id", (req, res) => { //delete patients from id
  con.query(`delete from patients where id = ${req.params.id}`,
    (err, result) => {
      if (err) res.statusCode(404).send(err.message);
      if (result.affectedRows > 0) { //checks if the patient to delete exsists by checking row value
        res.send({
          status: "success",
...
app.put("/patients/:id", (req, res) => { //put methid to update
  const {
    patientName,
    email,
    password,
...
//Login API
//Single API For both doctor and patient, all we did was to update the SQL query to doctors/patients
app.post("/login", (req, res) => {
  const { email, password, role } = req.body;
  if (role === "patient") { //role checks either patient or doctor
    con.query(
...
app.post("/patient/problem", (req, res) => {
  ...
  con.query(
    ...
    (err, result) => {
      ...
      res.send({
        status: "success",
        data: "Your problem was added successfully",
      });
    }
  );
});
